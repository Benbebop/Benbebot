...
function c.get()
	local tbl = {}
	local section = false
	for i in appdata.lines("global.ini") do
		local s = i:match("^%s*%[(.-)%]%s*$")
		local k, v = i:match("^(.-)=(.-)$")
...
function c.checkSection(section)
	local config = appdata.get("global.ini", "rb")
	local exists = false
	repeat
...
function c.setKey(section, key, value)
	...
	until not p
	config:close()
	appdata.write("global.ini", str:gsub("\n$", ""))
end

function c.verify()
	local o = c.get()
	local v = io.open("tables/global.ini.default", "rb")
	appdata.write("global.ini", v:read("*a"))
	v:close()
